...
2. `AZOTH_01.1_Tratado_da_Alquimia_v0.9.md`
3. `AZOTH_01.2_Engenharia_dos_Circulos_v0.1.md`
4. `AZOTH_01.3_Taxonomia_das_Transmutacoes_v0.2.md`
5. `AZOTH_01.4_Pesquisa_Alquimica_v0.2.md`
6. `AZOTH_01.5_Academias_de_Alquimia_v0.2.md`
...
Ciência, engenharia, LET, arquitetura das ETs e Teoria das Transmutações.

Deve descrever como a alquimia é estudada, descrita, ensinada e aplicada pelas civilizações, incluindo Linguagem das Especificações da Transmutação, arquitetura, avaliação, evolução de ETs e teoria científica das transmutações, sem redefinir a MAA.

### 01.2 — Engenharia dos Círculos

Projeto, otimização e composição de especificações.

Deve detalhar construção técnica de círculos, composição, otimização, legibilidade e validação formal, sem alterar a Gramática Geométrica.

### 01.3 — Taxonomia das Transmutações

Classificação formal das transmutações, operações e técnicas.

Deve organizar tipos de transmutação, famílias técnicas e padrões de aplicação a partir da Teoria das Transmutações e dos Domínios, Operações e Modificadores definidos pela MAA.

### 01.4 — Pesquisa Alquímica

Método científico, validação, Convergência Experimental e evolução da alquimia.

Deve descrever como descobertas, experimentos, validações e Convergência Experimental evoluem dentro dos limites da ciência incompleta do universo.

### 01.5 — Academias de Alquimia

Instituições, especializações, linhas de pesquisa e catalogação científica de ETs.

Deve organizar as Academias como instituições de pesquisa e ensino, sem torná-las fontes da Verdade completa.
