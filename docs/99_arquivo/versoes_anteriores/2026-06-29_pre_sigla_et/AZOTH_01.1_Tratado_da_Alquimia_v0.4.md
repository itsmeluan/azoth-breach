...
**Especificação** — Descrição formal da transmutação escrita na Linguagem Universal da Alquimia.
...
Conhecimento científico corresponde ao conjunto de explicações, modelos e especificações cuja validade foi demonstrada de forma consistente e reproduzível.

Uma hipótese não se torna conhecimento científico por parecer plausível.

Ela somente passa a integrar o corpo científico da alquimia quando produz resultados compatíveis com a MAA e pode ser reproduzida sob condições equivalentes.
...
A Engenharia Alquímica constitui o ramo da alquimia responsável pelo projeto, desenvolvimento, validação, otimização e manutenção de Especificações de Transmutação (ETs).

Neste Tratado, a sigla `ET` designa uma Especificação da Transmutação em contexto de engenharia.
...
Seu objeto de trabalho é a Especificação de Transmutação.

### 3.2 Especificação de Transmutação (ET)

Uma Especificação de Transmutação (ET) constitui a descrição formal de uma transmutação escrita utilizando a Linguagem Universal da Alquimia.
...
Nenhuma ET deve ser considerada concluída sem validação experimental.

A validação verifica se sua execução produz resultados compatíveis com o projeto especificado.

Mesmo ETs plenamente validadas permanecem sujeitas à Incerteza Fundamental da Transmutação.

A validação demonstra apenas que a distribuição de resultados permanece compatível com o comportamento previsto pela MAA.
...
Consiste em produzir Especificações de Transmutação cada vez mais precisas, mais eficientes e mais próximas da melhor descrição possível da transformação desejada.
...
- Desenvolver o Capítulo 4.
- Definir a organização interna dos capítulos posteriores.
- Definir separação entre ciência conhecida pelas Academias e Verdade objetiva da MAA.
- Definir linguagem didática sem transformar o documento em Codex Veritatis.
...
- v0.4: inclusão do Capítulo 3 — Engenharia Alquímica, com definição, Especificação de Transmutação, objetivos, processo de projeto, modularidade, ambiguidade, Precisão Alquímica, complexidade, otimização, validação e evolução da engenharia alquímica.
